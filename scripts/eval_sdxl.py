...
from src.config.sdxl_config import SDXLConfig
from src.eval.evaluator import Evaluator
from src.pipeline.custom_sdxl_pipeline import load_deradiff_sdxl_pipeline
...
if __name__ == "__main__":
    ...
    custom_pipe = load_deradiff_sdxl_pipeline(
    ...
    test_prompts = load_yaml_file(SDXLConfig.test_prompts_path)["test_prompts"]
    ...
    with torch.no_grad():
        for i, p in enumerate(test_prompts, start=1):
            im = gen(
                ...
                num_inference_steps=SDXLConfig.num_inference_steps,
                seed=SDXLConfig.seed,
                output_dir=args.output_dir,
                save_image=args.save_images
            )
            evaluator.score_image(im, p)

    evaluator.save_results(SDXLConfig.model_type, args.Lambda, args.Beta)
